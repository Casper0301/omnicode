...
metadata: 
  ...
  modified: 2026-08-15T22:23:06.000Z
---
...
**Current policy:** no Gemini or Antigravity lane. Cross-vendor review routes to Grok first, Codex second.
...
- **`lane-pick <class>`** (`~/.local/bin/lane-pick`) resolves a task class to the strongest configured lane not on cooldown and not known to have dead credentials. Ladders live in `~/.claude/omnicode/ladders.json`; cooldowns in `~/.lanes/health.json`. Classes: `code` (grok→glm→codex→claude) · `correctness` (codex→glm→grok→claude) · `langchain` (dcode→glm→claude; codex skipped — same quota) · `longcontext` (glm→claude) · `research` (glm→grok→codex→claude) · `review` (glm→grok→codex→claude) · `ui` (claude→glm→codex→grok) · `architecture` (claude). `claude` means the current architect session, regardless of its actual provider.
...
- Correctness-critical/high-stakes → race-and-judge (Opus + codex + grok implement independently → Opus judges → **GLM-5.3** verifies). It fails closed unless candidates completed with independent verification and GLM returns `sound`.
- Architecture/migration/refactor → Fable architect + consult `fable-advisor` at the boundary.
- Context >200K → GLM-5.3 lane (1M context on the z.ai Coding Plan). GPT-5.6-Sol currently advertises a 272K cap with a 95% effective budget (about 258K), so it is not the long-context fallback.
...
User-invocable via `/rjv`. `~/.claude/workflows/race-and-judge.mjs`: Opus + Codex/GPT + Grok implement the same five-part spec in isolated worktrees; only `complete` candidates whose supervisor re-ran verification reach the judge; the workflow normalizes the winning path from its own candidate records; GLM-5.3 then adversarially verifies. The workflow returns an apply instruction only when GLM says `sound`. The architect applies the diff, inspects it, and re-runs verification.
...
- **Deep research:** use distinct GLM, Grok, and Codex angles through `lanes`, with URL+date citations or `WEB_UNAVAILABLE`; the architect decomposes and synthesizes. Do not add a Gemini lane.
...
- Active lanes: codex (GPT-5.6-Sol @ max, ChatGPT subscription), grok (Grok 4.5, xAI subscription), glm (GLM-5.3, z.ai Coding Plan), dcode (GPT-5.6-Sol through `openai_codex`; LangGraph/LangChain harness fit, not a new family). Gemini/Antigravity are retired and must not appear in routing or agent discovery.
